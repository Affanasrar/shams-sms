...
export function CollapsibleSidebar() {
 ...
 className="ml-auto rounded-xl text-slate-300 hover:bg-card/10 hover:text-white"
 >
 ...
 <div key={group.title}>
 {!collapsed && <p className="mb-2 px-3 text-[11px] font-semibold uppercase tracking-[0.28em] text-muted-foreground">{group.title}</p>}
 ...
 >
 <div className={cn("flex h-8 w-8 items-center justify-center rounded-xl", isActive ? "bg-card/10" : "bg-card/5")}>{item.icon}</div>
 {!collapsed && (
 <span className="flex-1 truncate font-medium">{item.label}</span>
 )}
 {!collapsed && item.badge ? (
 <span className="rounded-full bg-rose-50 dark:bg-rose-950/400/90 px-2 py-0.5 text-[10px] font-semibold text-white">{item.badge}</span>
 ) : null}
 </Link>
 )
 })}
 ...
 <div className="flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-card/5 p-3">
