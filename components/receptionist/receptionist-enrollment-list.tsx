...
export function ReceptionistEnrollmentList({ enrollments }: ReceptionistEnrollmentListProps) {
 ...
 <h2 className="mt-2 text-2xl font-semibold text-foreground">Active enrollments</h2>
 <p className="mt-2 text-sm text-muted-foreground">Search active enrollments by student, course, room, or schedule.</p>
 </div>
 ...
 className="w-full rounded-2xl border border-border bg-card py-3 pl-10 pr-4 text-sm shadow-sm outline-none transition focus:border-cyan-400 focus:ring-2 focus:ring-cyan-100"
 />
 </label>
 <button
 type="button"
 ...
 <tr key={enrollment.id} className="transition hover:bg-muted/70">
 <td className="px-6 py-4 align-top text-sm">
 <div className="font-semibold text-foreground">{enrollment.studentName}</div>
 <div className="text-xs text-muted-foreground">{enrollment.fatherName}</div>
 <div className="mt-2 text-xs font-mono text-muted-foreground">{enrollment.studentId}</div>
 </td>
 <td className="px-6 py-4 text-sm text-foreground">{enrollment.courseName}</td>
 <td className="px-6 py-4 text-sm text-foreground">{enrollment.slotDays} • {enrollment.slotTime}</td>
 <td className="px-6 py-4 text-sm text-foreground">{enrollment.roomName}</td>
 <td className="px-6 py-4 text-sm text-foreground">{new Date(enrollment.joiningDate).toLocaleDateString('en-US', { timeZone: 'Asia/Karachi' })}</td>
 </tr>
 ))
 )}
 </tbody>
