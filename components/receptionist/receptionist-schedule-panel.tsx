...
import { ChevronDown, ChevronUp, Search, List, CalendarDays, Grid, Sparkles, Calendar, LayoutGrid } from 'lucide-react'
...
export function ReceptionistSchedulePanel({ roomGroups, rawAssignments = [], rooms = [] }: ReceptionistSchedulePanelProps) {
  const [activeTab, setActiveTab] = useState<'matrix' | 'quick-finder' | 'forecast' | 'rooms'>('matrix')
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-[28px] border border-border bg-card/90 p-3 shadow-xs">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => setActiveTab('quick-finder')}
            className={`inline-flex items-center gap-2 rounded-2xl px-4 py-2.5 text-sm font-semibold transition-all ${
              activeTab === 'quick-finder'
      ...
      {/* Main Tab Views */}
      {activeTab === 'matrix' && rawAssignments.length > 0 && (
        <ScheduleMatrixGrid
          assignments={rawAssignments as MatrixAssignment[]}
          rooms={rooms}
          baseEnrollUrl="/receptionist/enrollment/new"
        />
      )}

      {activeTab === 'quick-finder' && rawAssignments.length > 0 && (
        <QuickSeatFinder
          assignments={rawAssignments}
          enrollBasePath="/receptionist/enrollment/new"
          title="Receptionist Seat Finder & Intake"
          description="Find open chairs and class times instantly during student visits or inquiries"
        />
      )}

      {activeTab === 'forecast' && rawAssignments.length > 0 && (
        <VacancyForecastPanel assignments={rawAssignments} />
      )}
        ...
        <div className="space-y-6">
          <div className="grid gap-4 xl:grid-cols-[1.5fr_1fr_1fr]">
            <div className="rounded-[2rem] border border-border bg-card p-6 shadow-sm">
              ...
              <h2 className="mt-2 text-2xl font-semibold text-foreground">Find rooms, timings, or students</h2>
              <p className="mt-2 text-sm text-muted-foreground">The schedule is grouped by room and expands only when you need the details.</p>
              ...
              <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                <label className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full rounded-2xl border border-border bg-muted py-3 pl-10 pr-4 text-sm shadow-sm outline-none transition focus:border-cyan-400 focus:bg-card focus:ring-2 focus:ring-cyan-100"
                  />
                </label>
                <button
                  type="button"
              ...
              {activeQuery && (
                <div className="mt-3 flex flex-col gap-2 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
                  <span>Filtering by <strong className="text-foreground">{activeQuery}</strong></span>
            ...
            <Link href="/receptionist/students" className="rounded-[2rem] border border-border bg-card p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-cyan-200 hover:shadow-lg">
              <div className="flex h-full flex-col justify-between gap-4">
                <div>
                  <p className="text-sm text-muted-foreground">Admissions</p>
                  <p className="mt-2 text-xl font-semibold text-foreground">Student list</p>
                  <p className="mt-2 text-sm text-muted-foreground">Jump back to the student directory for lookup and filtering.</p>
                </div>
                <List className="text-cyan-600" />
              </div>
            </Link>

            <Link href="/receptionist/enrollment" className="rounded-[2rem] border border-border bg-card p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-cyan-200 hover:shadow-lg">
              <div className="flex h-full flex-col justify-between gap-4">
                <div>
                  <p className="text-sm text-muted-foreground">Enrollments</p>
                  <p className="mt-2 text-xl font-semibold text-foreground">Enrollment list</p>
                  <p className="mt-2 text-sm text-muted-foreground">Review active placements by student, course, and room.</p>
                </div>
          ...
          <div className="space-y-4">
            {filteredRooms.length === 0 ? (
              ...
              filteredRooms.map((room) => {
                ...
                return (
                  <article key={room.roomName} className="overflow-hidden rounded-[2rem] border border-border bg-card shadow-sm">
                    <button
                      type="button"
                      onClick={() => toggleRoom(room.roomName)}
                      className="flex w-full flex-col gap-4 border-b border-border bg-muted/80 px-6 py-5 text-left transition hover:bg-muted/95 sm:flex-row sm:items-center sm:justify-between"
                    >
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.22em] text-cyan-700">Room summary</p>
                        <h2 className="mt-2 text-2xl font-semibold text-foreground">{room.roomName}</h2>
                    ...
                    <div className="p-6 pt-5">
                      <button
                        type="button"
                        onClick={() => toggleRoom(room.roomName)}
                        className="inline-flex w-full items-center justify-between rounded-2xl border border-border bg-card px-4 py-3 text-left text-sm font-semibold text-foreground transition hover:border-cyan-200 hover:bg-cyan-50/60"
                      >
                      ...
                      {roomOpen && (
                        <div className="mt-4 space-y-4">
                          {room.timings.map((timing) => {
                            ...
                            return (
                              <div key={timing.slotId} className="rounded-[1.75rem] border border-border bg-muted p-5">
                                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                                  <div>
                                    <p className="text-sm font-medium text-cyan-700">{timing.days}</p>
                                    <h3 className="mt-1 text-lg font-semibold text-foreground">{timeLabel}</h3>
                                    <p className="mt-2 text-sm text-muted-foreground">
                                ...
                                <button
                                  ...
                                  className="mt-5 inline-flex w-full items-center justify-between rounded-2xl border border-border bg-card px-4 py-3 text-left text-sm font-semibold text-foreground transition hover:border-cyan-200 hover:bg-cyan-50/60"
                                >
                                ...
                                {timingOpen && (
                                  <div className="mt-4 space-y-4">
                                    {timing.assignments.map((assignment) => (
                                      <div key={assignment.id} className="rounded-[1.5rem] border border-border bg-card p-4">
                                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                          <div>
                                            <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Course</p>
                                            <p className="mt-1 text-lg font-semibold text-foreground">{assignment.courseName}</p>
                                          </div>
                                          <div className="rounded-2xl bg-muted px-3 py-2 text-sm font-semibold text-foreground shadow-sm">
                                            Enrolled: {assignment.enrolledCount}
                                          </div>
                                        </div>

                                        <div className="mt-4 rounded-2xl bg-muted p-4 text-sm text-foreground">
                                          {assignment.students.length === 0 ? (
                                            <p className="text-muted-foreground">No enrolled students yet.</p>
                                          ) : (
                                            <div className="space-y-3">
                                              {assignment.students.map((student) => (
                                                <div key={student.id} className="flex flex-col gap-1 rounded-2xl border border-border bg-card p-3 sm:flex-row sm:items-center sm:justify-between">
                                                  <div>
