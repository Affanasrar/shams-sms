...
export default function ReceptionistShell({ children }: { children: ReactNode }) {
 ...
 return (
 <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(14,165,233,0.14),transparent_30%),linear-gradient(180deg,#f8fafc_0%,#eff6ff_100%)] text-foreground">
 <aside className="fixed left-0 top-0 z-30 hidden h-screen w-80 flex-col border-r border-border/80 bg-slate-950 text-slate-100 shadow-[16px_0_40px_-28px_rgba(2,6,23,0.9)] backdrop-blur-xl xl:flex">
 ...
 <p className="text-sm text-muted-foreground">Admissions, enrollments, schedules</p>
 </div>
 </div>
 </div>

 <div className="mt-5 rounded-3xl border border-white/10 bg-card/5 p-4">
 ...
 isActive
 ? 'bg-card text-foreground shadow-[0_10px_30px_-18px_rgba(255,255,255,0.6)]'
 : 'text-slate-300 hover:bg-card/8 hover:text-white'
 }`}
 aria-current={isActive ? 'page' : undefined}
 >
 <span className={`flex h-10 w-10 items-center justify-center rounded-2xl transition ${isActive ? 'bg-slate-950/5 text-cyan-600' : 'bg-card/5 text-slate-300 group-hover:bg-card/10 group-hover:text-white'}`}>
 {item.icon}
 </span>
 <span className="min-w-0 flex-1">
 <span className="block font-semibold">{item.label}</span>
 <span className={`block text-xs ${isActive ? 'text-muted-foreground' : 'text-muted-foreground'}`}>{item.description}</span>
 </span>
 ...
 <div className="flex items-center gap-3 rounded-3xl border border-white/10 bg-card/5 p-3">
 <UserButton afterSignOutUrl="/" />
 <div className="min-w-0">
 <p className="truncate text-sm font-medium text-white">Receptionist</p>
 <p className="truncate text-xs text-muted-foreground">Account access</p>
 </div>
 </div>
 </div>
 </aside>

 <header className="sticky top-0 z-40 border-b border-border/80 bg-card/90 backdrop-blur-xl xl:hidden">
 <div className="flex items-center justify-between px-4 py-3 sm:px-6">
 <div className="flex items-center gap-3">
 <button
 onClick={() => setIsMobileMenuOpen(true)}
 aria-label="Open menu"
 className="rounded-2xl border border-border bg-card p-2.5 text-foreground transition hover:border-cyan-300 hover:text-cyan-700"
 >
 <Menu size={20} />
 </button>
 ...
 <div>
 <p className="text-[11px] uppercase tracking-[0.26em] text-muted-foreground">Receptionist desk</p>
 <p className="text-sm font-semibold text-foreground">{currentRouteLabel}</p>
 </div>
 </div>
 <UserButton afterSignOutUrl="/" />
 </div>
 ...
 <div className="scrollbar-hide absolute left-0 top-0 h-full w-[88vw] max-w-sm overflow-y-auto border-r border-border bg-card shadow-2xl">
 ...
 <div>
 <p className="text-xs uppercase tracking-[0.26em] text-muted-foreground">Receptionist desk</p>
 <p className="font-semibold text-foreground">Shams SMS</p>
 </div>
 </div>
 <button
 onClick={() => setIsMobileMenuOpen(false)}
 className="rounded-2xl border border-border p-2 text-muted-foreground transition hover:bg-muted"
 ...
 isActive ? 'bg-cyan-50 text-cyan-700' : 'text-muted-foreground hover:bg-muted hover:text-foreground'
 }`}
 aria-current={isActive ? 'page' : undefined}
 >
 <span className={`flex h-10 w-10 items-center justify-center rounded-2xl ${isActive ? 'bg-cyan-100' : 'bg-muted'}`}>
 {item.icon}
 </span>
 <span>
 <span className="block font-semibold">{item.label}</span>
 ...
 <div className="flex items-center gap-3 rounded-3xl border border-border p-3">
 <UserButton afterSignOutUrl="/" />
 <div>
 <p className="text-sm font-medium text-foreground">Receptionist</p>
