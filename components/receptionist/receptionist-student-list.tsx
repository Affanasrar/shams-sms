...
export function ReceptionistStudentList({ students }: ReceptionistStudentListProps) {
 ...
 <h2 className="mt-2 text-2xl font-semibold text-foreground">Registered students</h2>
 <p className="mt-2 text-sm text-muted-foreground">Search the student directory by ID, name, father name, or phone.</p>
 </div>
 ...
 <tr key={student.id} className="transition hover:bg-muted/70">
 <td className="px-6 py-4 font-mono text-sm font-medium text-foreground">{student.studentId}</td>
 <td className="px-6 py-4 text-sm font-medium text-foreground">{student.name}</td>
 <td className="px-6 py-4 text-sm text-muted-foreground">{student.fatherName}</td>
 <td className="px-6 py-4 text-sm text-muted-foreground">{student.phone}</td>
 <td className="px-6 py-4 text-sm text-muted-foreground">{new Date(student.admission).toLocaleDateString('en-US', { timeZone: 'Asia/Karachi' })}</td>
 </tr>
 ))
 )}
 </tbody>
