      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-6">
        <div className="flex gap-3 flex-col md:flex-row md:items-end">
          <div className="flex-1">
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Search Classes</label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
          </div>
      ...
      <div className="space-y-4">
        {filteredCourses.length > 0 ? (
          filteredCourses.map((course) => (
            <div key={course.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              {/* Course Header with Toggle */}
              <button
                onClick={() => toggleCourse(course.id)}
                className="w-full px-6 py-4 flex justify-between items-start hover:bg-gray-50 dark:hover:bg-slate-800/60 transition-colors border-b border-gray-100 dark:border-slate-800"
              >
                <div className="flex-1 text-left">
                  <h2 className="text-lg font-bold text-gray-900 dark:text-slate-100">{course.name}</h2>
                  <p className="text-sm text-gray-600 dark:text-slate-400 mt-1">
                    Total Slots: {course.classes.length}
                  </p>
                </div>
                <div className="flex items-center gap-4">
                  <div className="p-2 hover:bg-gray-200 dark:hover:bg-slate-700 rounded-lg transition-colors">
                      ...
                      {course.classes
                        ...
                        <div
                          ...
                          className={`p-5 rounded-lg border transition-all ${
                            cls.isMarked
                              ? 'bg-white dark:bg-slate-800/60 border-green-200 dark:border-green-900/50 hover:border-green-400 dark:hover:border-green-700 hover:shadow-md'
                        ...
                        >
                          ...
                          <div className="flex justify-between items-start mb-3">
                            <div className="flex-1">
                              <p className="text-sm text-gray-500 dark:text-slate-400">{cls.slot.days}</p>
                          ...
                          {/* Room Info */}
                          <div className="text-xs text-gray-600 dark:text-slate-400 mb-3 space-y-1">
                          ...
                          <div className={`p-3 rounded-lg mb-4 ${
                            cls.isMarked 
                              ...
                              : 'bg-gray-100 dark:bg-slate-800/60 border border-gray-300 dark:border-slate-700'
                          }`}>
                            {cls.isMarked ? (
                              <div>
                                <div className="text-sm font-bold text-gray-900 dark:text-slate-100 mb-2">Today's Attendance</div>
                                <div className="flex items-baseline gap-2">
                                  <div className="text-2xl font-bold text-green-600">{cls.presentCount}</div>
                                  <div className="text-sm text-gray-600 dark:text-slate-400">out of {cls.totalEnrolled} present</div>
                                </div>
                          ...
                          {/* Students Info */}
                          <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-slate-400 mb-4 pb-4 border-b border-gray-200 dark:border-slate-700">
                            <Users size={14} />
