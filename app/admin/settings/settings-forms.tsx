...
export function SettingsForms({ rooms, courses, slots, teachers }: Props) {
 ...
 icon={<Building2 size={20} className="text-indigo-600 dark:text-indigo-300"/>}
 >
 ...
 icon={<BookOpen size={20} className="text-emerald-600 dark:text-emerald-300"/>}
 >
 ...
 <div className="w-12 h-12 bg-card/10 rounded-xl flex items-center justify-center text-blue-400 mb-4">
 <LinkIcon size={24} />
 </div>
 ...
 <div className="text-xs bg-blue-50 dark:bg-blue-950/400/20 text-blue-300 px-3 py-1 rounded-full w-fit">
 Step 4 of 4
 </div>
 </div>

 {/* Right Side: Form */}
 <div className="md:w-2/3 bg-card/5 p-6 rounded-xl border border-white/10">
 ...
 className="w-full bg-blue-600 hover:bg-blue-50 dark:bg-blue-950/400 text-white font-bold py-3 rounded-lg transition-all shadow-lg hover:shadow-blue-500/30 flex justify-center items-center gap-2"
 >
 ...
 {linkState?.message && (
 <div className="bg-green-50 dark:bg-green-950/400/20 border border-green-500/50 text-green-200 px-4 py-3 rounded-lg text-sm flex items-center gap-2">
 <CheckCircle2 size={16} /> {linkState.message}
 </div>
 )}
 {linkState?.error && (
 <div className="bg-red-50 dark:bg-red-950/400/20 border border-red-500/50 text-red-200 px-4 py-3 rounded-lg text-sm flex items-center gap-2">
 ...
 icon={<DollarSign size={20} className="text-blue-600 dark:text-blue-300"/>}
 >
 ...
 icon={<History size={20} className="text-purple-600 dark:text-purple-300"/>}
 >
 <div className="space-y-4">
 {courses.map(course => (
...
function FormCard({ title, description, icon, children }: { title: string, description: string, icon: React.ReactNode, children: React.ReactNode }) {
 return (
 <div className="bg-card border border-gray-100 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300 overflow-hidden">
 <div className="p-5 border-b border-gray-50 bg-muted/50 flex items-start justify-between">
 <div>
 <h4 className="font-bold text-foreground ">{title}</h4>
 <p className="text-xs text-muted-foreground mt-1">{description}</p>
 </div>
 <div className="bg-card p-2 rounded-lg border border-gray-100 shadow-sm">{icon}</div>
 </div>
 <div className="p-5">
 {children}
 </div>
...
function InputGroup({ label, name, type = "text", placeholder }: { label: string, name: string, type?: string, placeholder?: string }) {
 ...
 required 
 className="w-full px-3 py-2 bg-card border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-black/5 dark:focus:ring-white/10 focus:border-black dark:focus:border-slate-500 transition-all"
 />
 </div>
 )
}
...
function SelectGroup({ label, children }: { label: string, children: React.ReactNode }) {
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1.5">{label}</label>
 <div className="w-full px-3 py-2 bg-card border border-border rounded-lg text-sm focus-within:ring-2 focus-within:ring-black/5 dark:focus-within:ring-white/10 focus-within:border-black dark:focus-within:border-slate-500 transition-all">
 {children}
 </div>
 </div>
 )
...
function CourseFeeUpdater({ course }: { course: any }) {
 ...
 <span className="text-sm text-muted-foreground ">Current: PKR {course.baseFee.toLocaleString()}</span>
 </div>
 ...
 min="0"
 step="0.01"
 className="flex-1 px-3 py-2 bg-card border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
 />
 <button
 disabled={feePending}
...
function CourseFeeHistory({ course }: { course: any }) {
 ...
 return (
 <div className="border border-border rounded-lg p-4 bg-card ">
 <h5 className="font-semibold text-foreground mb-3">{course.name}</h5>
 
 {loading ? (
 <div className="text-sm text-muted-foreground ">Loading history...</div>
 ) : history.length === 0 ? (
 <div className="text-sm text-muted-foreground ">No fee changes yet</div>
 ) : (
 <div className="space-y-2 max-h-40 overflow-y-auto">
 {history.map((change: any) => (
 <div key={change.id} className="text-xs bg-muted border border-gray-100 p-2 rounded">
 <div className="flex justify-between">
 <span className="text-foreground ">PKR {Number(change.oldFee).toLocaleString()} → PKR {Number(change.newFee).toLocaleString()}</span>
...
function DarkSelectGroup({ label, children }: { label: string, children: React.ReactNode }) {
 ...
 <div className="w-full px-3 py-3 bg-card/5 border border-white/10 rounded-lg text-sm text-white focus-within:ring-2 focus-within:ring-blue-500/50 focus-within:border-blue-500 transition-all">
 {children}
 </div>
 </div>
 )
