...
export function SmsSender({ students, courseSlots, onSent }: Props) {
 ...
 <p className="text-xs text-muted-foreground ">Sent to every selected student</p>
 </div>
 </div>
 ...
 <div>
 <h3 className="text-sm font-semibold text-foreground ">Audience Filters</h3>
 <p className="text-xs text-muted-foreground ">Narrow recipients by course or time slot</p>
 </div>
 </div>
 ...
 className="flex items-center gap-1 rounded-full border border-border bg-card px-3 py-1 text-xs font-medium text-muted-foreground transition hover:border-rose-200 dark:border-rose-900/50 hover:bg-rose-50 dark:bg-rose-950/40 hover:text-rose-600 dark:hover:border-rose-800 dark:hover:bg-rose-950/30 dark:hover:text-rose-400"
 >
 <X size={12} /> Clear
 </button>
 )}
 ...
 <label className="block text-xs font-semibold uppercase tracking-widest text-muted-foreground ">Course</label>
 <div className="relative">
 <select
 value={selectedCourse}
 onChange={e => handleCourseChange(e.target.value)}
 className="w-full appearance-none rounded-2xl border border-border bg-muted px-4 py-3 pr-10 text-sm text-foreground outline-none transition focus:border-violet-400 focus:ring-2 focus:ring-violet-100 dark:focus:ring-violet-900/40"
 >
 ...
 <ChevronDown size={15} className="pointer-events-none absolute right-3.5 top-3.5 text-muted-foreground" />
 </div>
 </div>

 {/* Slot select */}
 <div className="space-y-1.5">
 <label className="block text-xs font-semibold uppercase tracking-widest text-muted-foreground ">Time Slot</label>
 <div className="relative">
 <select
 value={selectedCourseSlot}
 onChange={e => handleSlotChange(e.target.value)}
 disabled={!selectedCourse}
 className="w-full appearance-none rounded-2xl border border-border bg-muted px-4 py-3 pr-10 text-sm text-foreground outline-none transition focus:border-violet-400 focus:ring-2 focus:ring-violet-100 dark:focus:ring-violet-900/40 disabled:cursor-not-allowed disabled:opacity-50"
 >
 ...
 {/* Channel select */}
 <div className="space-y-1.5">
 <label className="block text-xs font-semibold uppercase tracking-widest text-muted-foreground ">Delivery Channel</label>
 <div className="relative">
 <select
 value={channel}
 ...
 <div>
 <h3 className="text-sm font-semibold text-foreground ">Send Results</h3>
 ...
 result.success
 ? 'border-emerald-200 dark:border-emerald-900/50 bg-emerald-50 dark:border-emerald-800/40 dark:bg-emerald-950/30'
 : 'border-rose-200 dark:border-rose-900/50 bg-rose-50 dark:border-rose-800/40 dark:bg-rose-950/30'
 }`}
 >
 {result.success
 ...
 <span className="ml-2 text-xs text-muted-foreground ">{result.message}</span>
 </div>
 </div>
 )
 })}
 ...
 <p className="text-xs text-muted-foreground ">{selectedCount} of {visibleCount} selected</p>
 </div>
 </div>
 <button
 type="button"
 ...
 className="w-full rounded-xl border border-border bg-card py-2 pl-3 pr-8 text-xs text-foreground outline-none transition placeholder:text-muted-foreground focus:border-sky-400 focus:ring-1 focus:ring-sky-100 dark:focus:ring-sky-900/40"
 />
 {searchQuery && (
 <button onClick={() => setSearchQuery('')} className="absolute right-2.5 top-2 text-muted-foreground hover:text-muted-foreground">
 <X size={13} />
 </button>
 )}
 </div>
 ...
 <div>
 <p className="text-sm font-semibold text-foreground ">Search or filter to find recipients</p>
 <p className="mt-1 text-xs text-muted-foreground ">Type a name or ID in the search box,<br />or select a course above.</p>
 </div>
 </div>
 ...
 className="mt-0.5 h-4 w-4 shrink-0 rounded border-border text-indigo-600 dark:text-indigo-300 focus:ring-indigo-500"
 />
 <div className="min-w-0 flex-1">
 <div className="flex items-start justify-between gap-2">
 <div className="min-w-0">
 <p className="truncate text-sm font-semibold text-foreground ">{student.name}</p>
 <p className="text-xs text-muted-foreground ">{student.studentId} · s/o {student.fatherName}</p>
 </div>
 ...
 ? 'border-emerald-200 dark:border-emerald-900/50 bg-emerald-50 dark:border-emerald-800/40 dark:bg-emerald-950/30 text-emerald-800 dark:text-emerald-200'
 : 'border-rose-200 dark:border-rose-900/50 bg-rose-50 dark:border-rose-800/40 dark:bg-rose-950/30 text-rose-800 dark:text-rose-200'
 }`}>
 {feedback.type === 'success'
