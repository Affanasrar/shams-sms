...
export default function AdminLayoutClient({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
          ...
          {MOBILE_BOTTOM_NAV.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex-1 flex flex-col items-center justify-center text-xs py-1 ${active ? 'text-indigo-600' : 'text-gray-500'}`}
              >
                <Icon size={18} />
                <span className="mt-0.5">{item.label}</span>
                {active && <span className="w-1.5 h-1.5 rounded-full bg-indigo-600 mt-0.5" />}
              </Link>
            )
          })}
          <button
            onClick={() => setDrawerOpen(true)}
            className="flex-1 flex flex-col items-center justify-center text-xs py-1 text-gray-500"
          >
      ...
      {/* Main Content */}
      <main
        className="flex-1 transition-all duration-300 ease-in-out pt-14 md:pt-0 pb-20 md:pb-0"
        style={{
