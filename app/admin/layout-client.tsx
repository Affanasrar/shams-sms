...
export default function AdminLayoutClient({ children }: { children: React.ReactNode }) {
 ...
 className="flex h-9 w-9 items-center justify-center rounded-xl bg-card/10 text-white transition hover:bg-card/20 active:scale-95"
 ...
 <button onClick={() => setDrawerOpen(false)} className="p-2 rounded-lg hover:bg-card/10">
 <X size={18} />
 </button>
 </div>
 ...
 <p className="px-3 mb-2 text-[11px] font-semibold uppercase tracking-widest text-muted-foreground">{group.section}</p>
 ...
 active
 ? 'bg-indigo-50 dark:bg-indigo-950/400/20 text-white'
 ...
 <div className="text-xs text-muted-foreground">Account access</div>
 </div>
 </div>
 <ThemeToggle />
 </div>
 ...
 {/* Mobile Bottom Nav */}
 <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-card border-t border-border z-30" role="navigation" aria-label="Bottom navigation">
 ...
 key={item.href}
 href={item.href}
 className={`flex-1 flex flex-col items-center justify-center text-xs py-1 ${active ? 'text-indigo-600 dark:text-indigo-400' : 'text-muted-foreground '}`}
 >
 <Icon size={18} />
 ...
 <button
 onClick={() => setDrawerOpen(true)}
 className="flex-1 flex flex-col items-center justify-center text-xs py-1 text-muted-foreground "
 >
 ...
 {/* Main Content */}
 <main
 className="flex-1 min-h-screen max-w-full overflow-x-hidden transition-all duration-300 ease-in-out pt-14 md:pt-0 pb-20 md:pb-0 bg-muted/80 text-foreground "
 style={{
