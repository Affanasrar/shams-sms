    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Link
            href="/admin"
            className="rounded-full border border-slate-200 dark:border-slate-800 p-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition"
          >
            <ArrowLeft size={18} />
          </Link>
          <div>
      ...
      <div className="card-surface p-4 dark:bg-slate-900/80 dark:border-slate-800">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 py-2 text-sm outline-none focus:border-indigo-500 dark:border-slate-800 dark:bg-slate-950 dark:text-white"
          />
        </div>
      </div>
        ...
        <div className="divide-y divide-slate-200 dark:divide-slate-800">
          {pagedActivities.length === 0 ? (
            ...
            pagedActivities.map((activity) => (
              <div key={activity.id} className="p-4 sm:p-5 flex items-start gap-4 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                <div
        ...
        {filteredActivities.length > 0 && (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-slate-200 dark:border-slate-800 px-6 py-4 text-sm text-slate-600 dark:text-slate-400">
            <div>
              ...
              <span className="font-semibold text-slate-900 dark:text-white">{filteredActivities.length}</span> logs
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3.5 py-1.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-slate-700 dark:text-slate-300 disabled:opacity-40 text-xs font-medium transition hover:bg-slate-50"
              >
                Previous
              </button>
