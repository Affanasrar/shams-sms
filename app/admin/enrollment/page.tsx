...
export default async function EnrollmentIndex(props: Props) {
 ...
 <section className="rounded-[32px] border border-border/80 bg-card/80 p-6 shadow-[0_24px_70px_-28px_rgba(15,23,42,0.35)] backdrop-blur-xl sm:p-8">
 <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
 <div className="space-y-4">
 <div className="inline-flex items-center gap-2 rounded-full border border-indigo-200 dark:border-indigo-900/50 bg-indigo-50 dark:bg-indigo-950/40 px-3 py-1 text-sm font-medium text-indigo-700 dark:text-indigo-300">
 <Sparkles size={16} />
 Premium enrollment hub
 </div>
 <div>
 <h1 className="text-3xl font-semibold tracking-tight text-foreground">Active Enrollments</h1>
 ...
 <div className="rounded-2xl border border-border bg-muted/80 p-4 ">
 <div className="flex items-center gap-2 text-sm font-medium text-muted-foreground">
 <CalendarDays size={16} className="text-amber-600 dark:text-amber-300" />
 Active slots
 </div>
 ...
 <section className="rounded-[28px] border border-border/80 bg-card/80 p-4 shadow-[0_20px_60px_-28px_rgba(15,23,42,0.28)] backdrop-blur-xl sm:p-5">
 <EnrollmentFilters courses={courses} slots={slots} />
 </section>

 <section className="rounded-[28px] border border-border/80 bg-card/80 p-4 shadow-[0_20px_60px_-28px_rgba(15,23,42,0.28)] backdrop-blur-xl sm:p-5">
 <div className="mb-4 flex flex-col gap-2 border-b border-border/80 pb-4 sm:flex-row sm:items-center sm:justify-between">
 <div>
 <p className="text-sm font-semibold text-foreground">Enrollment roster</p>
 <p className="text-sm text-muted-foreground">A clear view of current placements and available options.</p>
 </div>
 <div className="text-sm text-muted-foreground">
 Showing <span className="font-semibold text-foreground">{rows.length}</span> active enrollment{rows.length !== 1 ? 's' : ''}
 </div>
 </div>

 {(courseId || slotId || searchQuery) && (
 <div className="mb-4 flex flex-wrap items-center gap-2 rounded-2xl border border-border bg-muted px-3 py-2 text-sm text-muted-foreground ">
 <span>Filtered by</span>
 {searchQuery && <span className="rounded-full bg-card px-2.5 py-1 font-medium text-foreground ">{searchQuery}</span>}
 {courseId && <span className="rounded-full bg-card px-2.5 py-1 font-medium text-foreground ">{courses.find((c) => c.id === courseId)?.name}</span>}
 {slotId && <span className="rounded-full bg-card px-2.5 py-1 font-medium text-foreground ">{slots.find((s) => s.id === slotId)?.days}</span>}
 <Link href="/admin/enrollment" className="ml-1 font-semibold text-indigo-600 dark:text-indigo-300 hover:text-indigo-700 dark:text-indigo-300">
 Clear all
 </Link>
 </div>
 )}
