...
export function ExtendCourseModal({ enrollmentId, courseName, isOpen, onClose }: ExtendCourseModalProps) {
 ...
 {state?.error && (
 <div className="text-red-600 dark:text-red-300 text-sm bg-red-50 dark:bg-red-950/40 p-2 rounded">
 {state.error}
 </div>
 )}

 {state?.success && (
 <div className="text-green-600 dark:text-green-300 text-sm bg-green-50 dark:bg-green-950/40 p-2 rounded">
 {state.message}
 </div>
 )}
 ...
 type="button"
 onClick={onClose}
 className="flex-1 px-4 py-2 border border-gray-300 rounded-md text-foreground hover:bg-muted"
 >
 Cancel
 </button>
 <button
