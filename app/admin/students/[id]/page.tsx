...
export default async function StudentProfilePage({ params }: { params: Promise<{ id: string }> }) {
 ...
 <Link href="/admin/students" className="rounded-full border border-white/15 p-2 transition hover:bg-card/10 hover:text-white">
 <ArrowLeft size={18} />
 </Link>
 <span className="text-xs uppercase tracking-[0.3em]">Student profile</span>
 </div>
 <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
 <div className="flex h-20 w-20 items-center justify-center rounded-3xl bg-card/10 text-3xl font-semibold text-white backdrop-blur">
 ...
 <div className="rounded-2xl border border-border bg-card p-5 shadow-sm dark:shadow-slate-950/50">
 <div className="flex items-center justify-between">
 <div>
 <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground ">Total fees</p>
 <p className="mt-2 text-2xl font-semibold text-foreground ">{formatCurrency(totalFees)}</p>
 </div>
 ...
 <p className="mt-2 text-2xl font-semibold text-emerald-600 dark:text-emerald-300">{formatCurrency(totalPaid)}</p>
 </div>
 ...
 <p className="mt-2 text-2xl font-semibold text-rose-600 dark:text-rose-300">{formatCurrency(totalDue)}</p>
 </div>
 <CreditCard className="h-6 w-6 text-rose-600 dark:text-rose-300" />
 </div>
 </div>
 <div className="rounded-2xl border border-border bg-card p-5 shadow-sm dark:shadow-slate-950/50">
 <div className="flex items-center justify-between">
 <div>
 <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground ">Communications</p>
 <p className="mt-2 text-2xl font-semibold text-foreground ">{smsLogs.length}</p>
 </div>
 ...
 <TabsList className="grid h-auto w-full grid-cols-4 gap-2 rounded-2xl bg-muted p-2">
 ...
 <Card className="overflow-hidden border-border p-0 shadow-sm dark:shadow-slate-950/50">
 <div className="border-b border-border bg-muted px-6 py-4">
 <h2 className="text-xl font-semibold tracking-tight text-foreground ">Fee Ledger</h2>
 <p className="text-sm text-muted-foreground ">The financial history for this student.</p>
 </div>
 ...
 <p className="text-2xl font-semibold text-emerald-600 dark:text-emerald-300">{formatCurrency(totalPaid)}</p>
 </div>
 <div className="rounded-2xl bg-muted p-4">
 <p className="text-sm text-muted-foreground ">Outstanding</p>
 <p className="text-2xl font-semibold text-rose-600 dark:text-rose-300">{formatCurrency(totalDue)}</p>
 </div>
 </div>
 ...
 return (
 <tr key={fee.id} className="border-b border-border transition hover:bg-muted/70 ">
 ...
 <td className="py-3 px-4 text-emerald-600 dark:text-emerald-300 font-medium">{formatCurrency(Number(fee.paidAmount))}</td>
 <td className="py-3 px-4 text-red-600 dark:text-red-300 font-medium">
 ...
 <Card className="overflow-hidden border-border p-0 shadow-sm">
 <div className="border-b border-border bg-muted px-6 py-4">
 <h3 className="text-lg font-semibold tracking-tight text-foreground">Exam Results</h3>
 </div>
 ...
 <div
 key={res.id}
 className="rounded-2xl border border-border p-4 transition hover:bg-muted"
 >
 <div className="flex justify-between items-start mb-3">
 <div>
 <h4 className="font-medium text-foreground">{res.course.name}</h4>
 ...
 Marks: <span className="font-semibold text-foreground">{Number(res.marks)} / {Number(res.total)}</span>
 </p>
 ...
 <Card className="overflow-hidden border-border p-0 shadow-sm">
 <div className="border-b border-border bg-muted px-6 py-4">
 <h3 className="text-lg font-semibold tracking-tight text-foreground">Summary</h3>
 </div>
 ...
 <p className="text-sm font-medium text-blue-600 dark:text-blue-300">Total SMS Sent</p>
 <p className="text-2xl font-bold text-blue-700 dark:text-blue-300">
 {smsLogs.filter(log => log.status === 'SENT').length}
 </p>
 </div>
 <div className="rounded-2xl bg-green-50 dark:bg-green-950/40 p-4">
 <p className="text-sm font-medium text-green-600 dark:text-green-300">This Week</p>
 ...
 <p className="text-sm font-medium text-yellow-600 dark:text-yellow-300">Skipped</p>
 ...
 <h3 className="mb-4 text-lg font-semibold text-foreground">Recent SMS Activity</h3>
 ...
 {smsLogs.map((log) => (
 <div key={log.id} className="rounded-2xl border border-border bg-muted p-4">
 ...
 {log.error && (
 <div className="mb-2 text-sm text-red-600 dark:text-red-300">
 Error: {log.error}
 </div>
 )}
