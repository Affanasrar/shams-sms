...
export default async function StudentList(props: Props) {
 ...
 <span className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-card/5 px-4 py-2 text-sm font-medium text-white/80">
 <Search size={16} />
 Search & filter live
 </span>
 </div>
 ...
 <p className="mt-2 text-2xl font-semibold text-foreground">{rows.length} students</p>
 <p className="mt-1 text-sm text-muted-foreground">{searchQuery ? `Filtered by "${searchQuery}"` : 'No search filter applied.'}</p>
 </div>
 <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
 <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">SMS reminders</p>
 <p className="mt-2 text-2xl font-semibold text-foreground">{smsEnabledStudents} active</p>
 <p className="mt-1 text-sm text-muted-foreground">Students receiving SMS updates.</p>
 </div>
 <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
 <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Admissions</p>
 <p className="mt-2 text-2xl font-semibold text-foreground">{recentAdmissions} in 30 days</p>
 <p className="mt-1 text-sm text-muted-foreground">Recent additions to the institute.</p>
 </div>
 </div>

 <StudentFilters />

 <div className="flex items-center justify-between gap-4 rounded-2xl border border-border bg-muted px-4 py-3 text-sm text-muted-foreground">
 <span>
 Showing <span className="font-semibold text-foreground">{rows.length}</span> student{rows.length !== 1 ? 's' : ''}
 {searchQuery && ` matching "${searchQuery}"`}
 </span>
 <Link href="/admin/students/new" className="inline-flex items-center gap-2 font-medium text-foreground transition hover:text-foreground">
