...
export function AttendanceForm({ classId, teacherId, enrollments }: Props) {
 ...
 return (
 <form onSubmit={handleSubmit} action={action} className="bg-card rounded-xl shadow-sm border overflow-hidden relative">
 ...
 {/* Date Header */}
 <div className="p-4 border-b bg-muted flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
 ...
 name="date" 
 defaultValue={today} 
 className="bg-transparent border-none focus:ring-0 p-0 text-foreground font-bold text-sm sm:text-base"
 />
 </div>
 ...
 {enrollments.map((enr) => (
 <div key={enr.student.id} ref={(el) => attachSwipe(el, enr.student.id)} className="p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 hover:bg-muted transition">
 <div className="flex items-center gap-3 flex-1">
 <div className="h-10 w-10 rounded-full bg-blue-100 text-blue-600 dark:text-blue-300 flex items-center justify-center font-bold text-sm flex-shrink-0">
 {enr.student.name.charAt(0)}
 </div>
 <div className="min-w-0 flex-1">
 <p className="font-bold text-foreground truncate">{enr.student.name}</p>
 <p className="text-sm text-blue-600 dark:text-blue-300 font-medium">ID: {enr.student.studentId}</p>
 <p className="text-xs text-muted-foreground">Father: {enr.student.fatherName}</p>
 </div>
 </div>
 ...
 <button type="button" onClick={() => cycleStatus(enr.student.id)} className={`px-4 py-3 rounded-lg font-bold text-sm transition-shadow shadow-sm ${statuses[enr.student.id] === 'PRESENT' ? 'bg-green-600 text-white' : statuses[enr.student.id] === 'ABSENT' ? 'bg-red-600 text-white' : 'bg-amber-50 dark:bg-amber-950/400 text-white'}`}>
 ...
 {/* Submit Footer */}
 <div className="p-4 bg-muted border-t flex flex-col gap-3">
 
 {/* Success/Error Messages */}
 {state?.message && (
 <div className="flex items-center gap-2 text-sm font-bold text-green-600 dark:text-green-300 bg-green-50 dark:bg-green-950/40 px-3 py-2 rounded-lg border border-green-100 dark:border-green-900/50">
 <CheckCircle2 size={16} /> {state.message}
 </div>
 )}
 {state?.error && (
 <div className="flex items-center gap-2 text-sm font-bold text-red-600 dark:text-red-300 bg-red-50 dark:bg-red-950/40 px-3 py-2 rounded-lg border border-red-100 dark:border-red-900/50">
