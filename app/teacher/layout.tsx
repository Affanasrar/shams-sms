...
export default function TeacherLayout({ children }: { children: React.ReactNode }) {
 ...
 {/* Desktop Sidebar */}
 <aside className="hidden md:flex flex-col w-72 bg-card border-r fixed top-0 left-0 bottom-0 z-10 safe-padding">
 ...
 <div className="text-xs text-muted-foreground">Easy classroom tools</div>
 </div>
 </div>
 ...
 <div className="text-xs text-muted-foreground">Sign out available</div>
 </div>
 </div>
 </div>
 </aside>
 ...
 {/* Mobile Topbar */}
 <header className="md:hidden fixed top-0 left-0 right-0 bg-card border-b z-20 safe-padding pt-safe">
 <div className="flex items-center justify-between px-4 py-3">
 <div className="flex items-center gap-3">
 <button onClick={() => { setIsMobileMenuOpen(true); try { navigator.vibrate?.(8) } catch {} }} aria-label="Open menu" className="p-2 rounded-md hover:bg-muted -ml-2">
 <Menu size={20} />
 </button>
 ...
 <div className="absolute left-0 top-0 bottom-0 w-72 bg-card shadow-xl p-4 overflow-auto">
 <div className="flex items-center justify-between mb-4">
 <div className="flex items-center gap-3">
 <div className="w-9 h-9 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-lg flex items-center justify-center text-white font-bold">T</div>
 <div className="font-bold">Teacher Portal</div>
 </div>
 <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 rounded-md hover:bg-muted"><X size={20} /></button>
 </div>
 ...
 {/* Mobile Bottom Nav */}
 <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-card border-t z-30" role="navigation" aria-label="Bottom navigation">
 ...
 {showInstall && (
 <div className="fixed bottom-6 left-6 right-6 md:right-6 md:left-auto z-50 bg-card border rounded-lg shadow-lg p-3 flex items-center gap-3 justify-between max-w-lg mx-auto">
 <div className="flex items-center gap-3">
 <div className="text-sm font-medium">Install Shams</div>
 <div className="text-xs text-muted-foreground">Get quick access to attendance</div>
 </div>
 <div>
 {deferredPrompt ? (
 <div className="flex items-center gap-2">
 ...
 <button onClick={() => setShowInstall(false)} className="px-2 py-1 text-sm text-muted-foreground">Dismiss</button>
 </div>
 ) : (
 // iOS fallback instructions
 <div className="flex items-center gap-2">
 <button onClick={() => setShowInstall(false)} className="px-3 py-1 bg-blue-600 text-white rounded">Got it</button>
 <div className="text-xs text-muted-foreground">Tap Share → Add to Home Screen</div>
 </div>
 )}
 </div>
 </div>
...
function NavLink({ href, icon, label, active }: { href: string, icon: React.ReactNode, label: string, active?: boolean }) {
 ...
 className={`flex items-center gap-3 px-4 py-3 min-h-12 rounded-lg transition-colors font-medium touch-target ${active ? 'bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300' : 'text-muted-foreground hover:bg-muted hover:text-blue-700 dark:text-blue-300'}`}
 >
 {icon}
 <span>{label}</span>
 </Link>
...
function MobileNavLink({ href, icon, label, onClick, active }: { href: string, icon: React.ReactNode, label: string, onClick: () => void, active?: boolean }) {
 ...
 onClick={onClick}
 aria-current={active ? 'page' : undefined}
 className={`flex items-center gap-3 px-4 py-3 min-h-12 rounded-lg transition-colors font-medium text-lg touch-target ${active ? 'bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300' : 'text-muted-foreground hover:bg-muted hover:text-blue-700 dark:text-blue-300'}`}
 >
 {icon}
 <span>{label}</span>
 </Link>
...
function MobileIcon({ href, label, icon, active }: { href: string; label: string; icon: React.ReactNode; active?: boolean }) {
 return (
 <Link href={href} aria-current={active ? 'page' : undefined} className={`flex-1 flex flex-col items-center justify-center text-xs min-h-12 touch-target ${active ? 'text-blue-600 dark:text-blue-300' : 'text-muted-foreground hover:text-blue-600 dark:text-blue-300'}`} onClick={() => { try { navigator.vibrate?.(6) } catch {} }}>
