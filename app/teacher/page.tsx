...
export default async function TeacherDashboard() {
 ...
 <Link href="/teacher/reports" className="inline-flex items-center gap-2 rounded-2xl bg-card/10 px-4 py-2 text-sm font-medium text-white transition hover:bg-card/20">
 ...
 <Clock className="text-indigo-600 dark:text-indigo-300" />
 <h2 className="text-xl font-semibold tracking-tight">My teaching schedule</h2>
 </div>

 <div className="grid gap-4 md:gap-6">
 {myClasses.length === 0 ? (
 <div className="rounded-[24px] border border-dashed border-border bg-muted p-10 text-center">
 <p className="text-muted-foreground">No classes assigned to you yet.</p>
 <p className="mt-2 text-sm text-indigo-600 dark:text-indigo-300">An admin needs to assign your classes first.</p>
 </div>
 ) : (
 myClasses.map((cls) => (
 <div key={cls.id} className="group rounded-[24px] border border-border bg-card/90 p-5 shadow-[0_16px_50px_-24px_rgba(15,23,42,0.35)] transition hover:-translate-y-0.5 hover:border-indigo-300">
 <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
 <div>
 <h3 className="text-lg font-semibold text-foreground">{cls.course.name}</h3>
 <div className="mt-1 flex items-center gap-2 text-sm text-muted-foreground">
 <MapPin size={16} /> {cls.slot.room.name}
 </div>
 </div>
 <span className="self-start rounded-full bg-indigo-50 dark:bg-indigo-950/40 px-3 py-1 text-xs font-semibold uppercase tracking-[0.25em] text-indigo-700 dark:text-indigo-300">
 {cls.slot.days}
 </span>
 </div>
...
function StatCard({ label, value }: { label: string, value: string | number }) {
 ...
 <div className="text-2xl font-semibold text-indigo-600 dark:text-indigo-300 sm:text-3xl">{value}</div>
 <div className="mt-2 text-xs font-semibold uppercase tracking-[0.28em] text-muted-foreground">{label}</div>
 </div>
 )
}