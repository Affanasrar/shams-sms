...
export default async function ReceptionistHomePage() {
 ...
 <Link href="/receptionist/students/new" className="inline-flex items-center gap-2 rounded-full bg-card px-4 py-2.5 text-sm font-semibold text-foreground transition hover:bg-muted">
 <UserPlus size={16} />
 New admission
 </Link>
 <Link href="/receptionist/enrollment/new" className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-card/5 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-card/10">
 <BookOpen size={16} />
 New enrollment
 </Link>
 </div>
 ...
 key={card.href}
 href={card.href}
 className="group rounded-[1.75rem] border border-border bg-card p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-cyan-200 hover:shadow-lg"
 >
